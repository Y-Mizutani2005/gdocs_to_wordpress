...
本ツールが生成するHTMLは、可能な限り WordPress のブロックエディタ (Gutenberg) と互換性を持つように設計されており、デフォルトではGutenbergの標準クラスが付与されます。
...
### 3. 見出し (Headings)
また、記事内の見出しには、自動的にIDが付与されます（目次からのアンカーリンク用）。
...
| 要素 | クラス名 | 備考 |
| :---- | :---- | :---- |
| `h2 \- h6` | `.wp-block-heading` | WordPress 6.x 以降の標準仕様に準拠。 |
| `h2 \- h4` | `(id 属性)` | 目次ジャンプ用の ID (toc-heading-{n}) が自動付与されます。 |

*※ Config.gs でクラス名のカスタマイズが可能です。*
...
| 要素 | クラス名 | 内容・説明 |
| :---- | :---- | :---- |
| `figure` | `.wp-block-image` | 画像ブロック全体のコンテナ。 |
| `figure` | `.size-large` | デフォルトのサイズ指定（大）。 |
| `img` | `.wp-image-{id}` | WordPress のメディアライブラリ ID に応じたクラス。 |
